...
const Categories = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen ">
      <div className="flex flex-col flex-1 max-w-6xl space-y-5  lg:space-y-0 lg:flex-row lg:space-x-10 sm:p-6 sm:my-2 sm:mx-4 sm:rounded-2xl">
          ...
          <div className="grid grid-cols-1 gap-4 mt-10 mb-10 sm:mb-0 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {data?.map((category: category, i: number) => (
              <Link key={i} to={`/category/${category.name}`}>
                <div className="relative flex flex-col items-center px-4 py-10 space-y-2 bg-fourth rounded-md cursor-pointer group sm:py-20 hover:bg-third hover:smooth-hover">
                  <img
